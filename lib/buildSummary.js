 ...
 * ~5KB, contains everything needed to diagnose most failures without reading anything else.
 */
 ...
 */
export function buildSummary(trace, network, stacks, cleanSteps) {
  ...
  const networkFailures = network.failures.map(f => ({
    callId: f.callId,
    method: f.method,
...
function extractRootCauseHints(errorMessage, networkFailures, consoleEntries) {
  const hints = [];
