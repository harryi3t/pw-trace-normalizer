 ...
 * @param {string|null} [options.resourcesDir] - absolute path to the trace resources/ dir
...
export function buildSummary(trace, network, stacks, cleanSteps, options = {}) {
  ...
  if (options.resourcesDir) {
    drillDown.resources = options.resourcesDir;
  }
  ...
  if (hasSignals) {
    result.signals = {
      ...
      hasUnmockedFlags: network.flagEvals.some(f => !f.mocked),
